...
export const fetchOrgUnits = async () => {
  const orgUnits = await ky
    .get(`${baseURL}/organisationUnits.json?fields=%3Aall&paging=false`, {
      ...options,
    })
    .json();
...
export const fetchOrgUnitLevels = async () => {
  const resp = await ky
    .get(
      `${baseURL}/organisationUnitLevels.json?fields=id,displayName~rename(name),level&paging=false&order=level:asc`,
      { ...options }
    )
    .json();
...
export const fetchOrgUnitGroups = async () => {
  const resp = await ky
    .get(
      `${baseURL}/organisationUnitGroups.json?fields=id,displayName~rename(name)&paging=false`,
      { ...options }
    )
    .json();
...
export const fetchACatchmentInUse = async (id) => {
  ...
  const resp = await ky
    .get(`${baseURL}/maps.json?filter=mapViews.orgUnitField:eq:${id}`, {
      ...options,
    })
    .json();
  return resp.maps;
...
export const fetchCurrentAttributes = async () => {
  const resp = await ky
    .get(
      `${baseURL}/attributes.json?fields=id,name&filter=valueType:eq:GEOJSON&filter=organisationUnitAttribute:eq:true&paging=false`,
      { ...options }
    )
    .json();
...
export const publishCatchment = async (body) => {
  ...
  try {
    ...
    const resp = await ky
      .post(`${baseURL}/attributes`, { json: body.payload, ...options })
      .json();
